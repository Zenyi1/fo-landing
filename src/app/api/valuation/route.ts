...
function heuristic(inputs: Inputs) {
  ...
  if (inputs.devStage !== "Approved") {
    ...
    const value =
      ...
      1.8 *
      jitter;
    return { value_musd: clamp(value, 1, 1500), markets: clamp((10 + (hash % 25)) * deals, 4, 53) };
  }
  const value =
    ...
    deals *
    jitter;
  return { value_musd: clamp(value, 2, 3000), markets: clamp((24 + (hash % 23)) * deals, 4, 53) };
}
...
Existing emerging-market deals already in place: ${inputs.emDeals}`;
  ...
  return {
    ...
    markets: clamp(parsed.markets, 4, 53),
  };
}
